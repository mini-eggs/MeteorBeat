 ...
 * CollisionParticle
 *
 * Once instantiated, give `Run` a rigid body to show collition explosion at the
 * rigid body's location.
 */
class CollisionParticle : Particle
{
  ...
  public override void Run(Rigidbody parent) 
  {
    ...
    var item = Instantiate(prefab, new Vector3(0, 0, 0), Quaternion.identity);
  ...
  public static Particle Get(ParticleType type)
  {
    switch (type)
    {
      case ParticleType.Collision:
        var col = ScriptableObject.CreateInstance<CollisionParticle>();
        col.Load("explosion_prefab");
        return col;
      case ParticleType.Direction: 
        var dir = ScriptableObject.CreateInstance<DirectionParticle>();
