...
public class LevelIntegration : MonoBehaviour
{
  ...
  void Start()
  {
    ...
    s.LoadSounds(this.gameObject); // First sound that plays always. Load all game sounds on start.
  ...
  void Update() 
  {
    ...
    {
      isRunning = false; // Book keeping, don't enter this conditional twice in a row.

      // Tell other components game is over.
      GameObject.FindGameObjectWithTag("Player").GetComponent<ShipIntegration>().UserHasWon();
